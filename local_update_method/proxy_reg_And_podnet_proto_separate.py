...
import torch
from local_update_method.global_and_online_model_proto import *

class LocalUpdate(object):
    def __init__(self, args, lr, local_epoch, device, batch_size, dataset=None, idxs=None, alpha=0.0,alpha_proto=0.0):
        self.lr=lr
    ...
    def train(self, net, idea_img,idea_label,delta=None):
        ...
        for iter in range(self.local_epoch):
            ...
            for batch_idx, (images, labels) in enumerate(self.ldr_train):
                ...
                net.zero_grad()
                logits,activation_loss,activation_loss_proto = model(images,online_target=True)
                log_probs = F.log_softmax(logits[:-proto_len]/self.args.temp, dim=1)
                log_probs_proto = F.log_softmax(logits[-proto_len:]/self.args.temp, dim=1)
                loss = self.loss_func(log_probs, labels)+self.alpha*activation_loss+self.alpha_proto*activation_loss_proto+self.args.cealpha_proto*self.loss_func(log_probs_proto, idea_label)
