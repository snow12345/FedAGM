...
from utils import DatasetSplit,SAM
import torch
...
class LocalUpdate(object):
    ...
    def train(self, net, delta=None):
        ...
        optimizer = SAM(model.parameters(), base_optimizer, lr=self.lr,momentum=self.args.momentum,weight_decay=self.args.weight_decay)
        epoch_loss = []
        for iter in range(self.local_epoch):
            ...
            for batch_idx, (images, labels) in enumerate(self.ldr_train):
                ...
                self.loss_func(model(images), labels).backward()
