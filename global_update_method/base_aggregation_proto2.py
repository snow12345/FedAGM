...
def filter_visualize(net,device,size=1):
    with torch.no_grad():
...
def GlobalUpdate(args,device,trainset,testloader,LocalUpdate):
    ...
    for epoch in range(args.global_epochs):
        ...
        if epoch==0:
            idea_img=filter_visualize(model,device,size=1)
            ...
            this_idea_img=filter_visualize(model,device,size=1)
            this_idea_label=torch.range(start=0,end=9,dtype=int).to(device)
            #idea_img+=torch.normal(mean=0,std=0.1,size=idea_img.shape).to(device)
            this_idea_img.clamp(0,1)
            idea_img=torch.cat((idea_img,this_idea_img))[-50:0]
            idea_label=torch.cat((idea_label,this_idea_label))[-50:0]
