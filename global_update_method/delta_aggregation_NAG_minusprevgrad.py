...
def GlobalUpdate(args,device,trainset,testloader,LocalUpdate):
    ...
    global_g_t = copy.deepcopy(global_delta)
    global_g_t_prev = copy.deepcopy(global_delta)
    ideal_model=copy.deepcopy(model)
    ideal_delta = copy.deepcopy(global_delta)
    for epoch in range(args.global_epochs):
        ...
        for key in global_delta.keys():
            sending_model_dict[key] += args.gamma * global_delta[key]
        ...
        for user in selected_user:
            ...
            for key in weight.keys():
                g_t[key] = this_tau*weight[key]-this_tau*sending_model_dict[key] 
            local_g_t.append(g_t)            
        ...
        FedAvg_weight = copy.deepcopy(sending_model_dict)
        global_g_t = copy.deepcopy(local_g_t[0])
        K_mean=sum(local_K)/len(local_K)
        for key in global_g_t.keys():
            for i in range(len(local_g_t)):
                if i==0:              
                    global_g_t[key] *= num_of_data_clients[i]
                else:                
                    global_g_t[key] += local_g_t[i][key] * num_of_data_clients[i]
            global_g_t[key] = global_g_t[key] / ( total_num_of_data_clients)
            FedAvg_weight[key] = FedAvg_weight[key] + global_g_t[key]
            global_delta[key] = args.gamma * global_delta[key] +  global_g_t[key] + args.lamb*(global_g_t[key]-global_g_t_prev[key])
            global_lr = args.g_lr        
        ...
        global_g_t_prev = copy.deepcopy(global_g_t)
        ...
        if args.compare_with_center>0:
            if args.compare_with_center ==1:
                ...
                for user in selected_user:
                    idxs+=dataset[user]
            before_ideal = ideal_model.state_dict()        
            ideal_sending_model = copy.deepcopy(ideal_model)
            ideal_sending_model_dict =ideal_sending_model.state_dict()
            for key in ideal_sending_model_dict.keys():
                ideal_sending_model_dict[key]+= args.gamma * ideal_delta[key]
            ideal_sending_model.load_state_dict(ideal_sending_model_dict)
            ...
            ideal_weight = centerupdate.train(net=copy.deepcopy(ideal_sending_model).to(device))
            for key in ideal_sending_model_dict.keys():
                ideal_weight[key] = ideal_weight[key]*this_tau +(1-this_tau)*ideal_sending_model_dict[key]
                ideal_delta[key] = ideal_weight[key]-before_ideal[key]
