...
def GlobalUpdate(args,device,trainset,testloader,LocalUpdate):
    ...
    global_delta_prev = copy.deepcopy(model.state_dict())
    for key in global_delta_prev.keys():
        global_delta_prev[key] = torch.zeros_like(global_delta[key])
    ...
    for epoch in range(args.global_epochs):
        ...
        for key in global_delta.keys():
            sending_model_dict[key] = sending_model_dict[key] -1 * args.gamma * global_delta[key] + args.tau * global_delta_prev[key]
        ...
        global_delta_prev = copy.deepcopy(global_delta)
