...
def GlobalUpdate(args,device,trainset,testloader,LocalUpdate):
    ...
    global_delta_ema = copy.deepcopy(model.state_dict())
    for key in global_delta.keys():
        global_delta[key] = torch.zeros_like(global_delta[key])
        global_delta_ema[key] = torch.zeros_like(global_delta[key])
    ...
    for epoch in range(args.global_epochs):
        ...
        for user in selected_user:
            ...
            weight, loss = local_setting.train(net=copy.deepcopy(model).to(device), delta=global_delta_ema)
        ...
        for key in global_delta.keys():
            ...
            global_delta_ema[key] = args.lamb * global_delta[key] + (1-args.lamb) * global_delta_ema[key]
