...
def GlobalUpdate(args,device,trainset,testloader,LocalUpdate):
    ...
    momentum_list=[]
    for el in args.agg_list:
        momentum_list.append(float(el))
    num_of_momentum = len(momentum_list)
    print("momentum_list")
    for el in momentum_list:
        print(el,type(el))
    print("num_of_momentum",num_of_momentum)
    ...
    v_t = []
    for i in range(num_of_momentum):
        v_t.append(copy.deepcopy(global_delta))
    mean_v_t = copy.deepcopy(global_delta)
    
    sending_model = copy.deepcopy(model)
    sending_model_dict = (sending_model.state_dict())
    ...
    for epoch in range(args.global_epochs):
        ...
        for user in selected_user:
            ...
            #    delta[key] = weight[key] -sending_model_dict[key]
        ...
        for key in FedAvg_weight.keys():
            ...
            global_delta[key] = FedAvg_weight[key] - sending_model_dict[key]
            mean_v_t[key] = 0
            for j in range(num_of_momentum):
                v_t[j][key] = (v_t[j][key])*(momentum_list[j]) + global_delta[key]
                mean_v_t[key]+=v_t[j][key]
            mean_v_t[key]/=num_of_momentum
            sending_model_dict[key]+=this_g_lr*mean_v_t[key]
        sending_model.load_state_dict(sending_model_dict)
        ...
        this_g_lr *=args.server_learning_rate_decay
