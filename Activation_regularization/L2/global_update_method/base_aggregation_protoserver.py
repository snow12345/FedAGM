...
def GlobalUpdate(args,device,trainset,testloader,LocalUpdate):
    ...
    for epoch in range(args.global_epochs):
        ...
        cl_label=torch.range(start=0,end=9,dtype=int).to(device)
        cl_label=cl_label.repeat(m)
        for i in range(m):
            tmp_model.load_state_dict(local_weight[i])
            if i==0:
                cl_img=filter_visualize(args.image_epochs,tmp_model,device,size=1)
            else:
                cl_img=torch.cat((cl_img,filter_visualize(args.image_epochs,tmp_model,device,size=1)),dim=0)
                
            
        
        wandb_dict['common_image']=[wandb.Image(cl_img, caption="global_epoch"+str(epoch))]
        #wandb.log({f"log probs-{i}": el[i] for i, el in enumerate(log_probs)},commit=False)
        ...
        if (args.t_sne==True) and (epoch%args.t_sne_freq==0):
            ...
            if epoch % args.print_freq == 0:
                ...
                print('Accuracy of the network on the 10000 test images: %f %%' % (
                        100 * correct / float(total)))
                before_acc_train.append(100 * correct / float(total))
        ...
        optimizer = optim.SGD(model.parameters(), lr=0.1,momentum=0,weight_decay=args.weight_decay)
        for i in range(10):
                model.zero_grad()
                log_probs = model(cl_img)
                loss = criterion(log_probs, cl_label)
                loss.backward()
                torch.nn.utils.clip_grad_norm_(model.parameters(), args.gr_clipping_max_norm)
                optimizer.step()            
        ...
        if (args.t_sne==True) and (epoch%args.t_sne_freq==0):
            ...
            if epoch % args.print_freq == 0:
                ...
                print('Accuracy of the network on the 10000 test images: %f %%' % (
                        100 * correct / float(total)))
                after_acc_train.append(100 * correct / float(total))

            model.train()            
        
        
        wandb_dict[args.mode + '_loss']= loss_avg
        wandb_dict[args.mode + "_acc_before"]=before_acc_train[-1]
        wandb_dict[args.mode + "_acc_after"]=after_acc_train[-1]
