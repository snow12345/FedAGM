...
class LocalUpdate(object):
    ...
    def train(self, net, predictor, delta=None, epoch=0):
        ...
        global_target_network = copy.deepcopy(net)
        for param_t in global_target_network.parameters():
        ...
        optimizer = optim.SGD(param_groups, lr=self.lr, momentum=self.args.momentum, weight_decay=self.args.weight_decay)
        epoch_loss = []
        for iter in range(self.local_epoch):
            batch_loss = []
            for batch_idx, (view1, view2, labels) in enumerate(self.ldr_train):
                ## get online target network
                online_target_network = copy.deepcopy(model)
                for param_t in online_target_network.parameters():
                    param_t.requires_grad = False
                ...
                with torch.no_grad():
                    ## Global target network view
                    targets_to_view2_from_global, _ = global_target_network(view1)
                    targets_to_view1_from_global, _ = global_target_network(view2)

                    targets_to_view2_from_online, _ = online_target_network(view1)
                    targets_to_view1_from_online, _ = online_target_network(view2)

                    ## online target network view


                consistency_loss = self.regression_loss(predictions1, targets_to_view1_from_global)
                consistency_loss += self.regression_loss(predictions2, targets_to_view2_from_global)
                consistency_loss += self.regression_loss(predictions1, targets_to_view1_from_online)
                consistency_loss += self.regression_loss(predictions2, targets_to_view2_from_online)
