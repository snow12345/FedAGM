...
class LocalUpdate(object):
    ...
    def train(self, net, delta=None, user=None):
        ...
        epoch_loss = []
        local_delta = self.local_deltas[user]
        for iter in range(self.local_epoch):
            ...
            for batch_idx, (images, labels) in enumerate(self.ldr_train):
                ...
                for n, p in net.named_parameters():
                    p = torch.flatten(p)
                    local_grad = torch.flatten(local_delta[n]).to(self.device)
            ...
            ## Update Local Delta
            for n, p in net.named_parameters():
                self.local_deltas[user][n] = (local_delta[n].cpu() - self.args.alpha * (p - fixed_params[n].detach()).cpu()).cpu()
