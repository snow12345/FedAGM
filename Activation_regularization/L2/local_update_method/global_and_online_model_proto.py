...
class dual_model(nn.Module):
    ...
    def get_activation_loss(self,online_outputs,global_outputs,x,x1):
    ...
    def forward(self, x,online_target=False):
        if online_target==False:
            ...
            x_proto=x[-self.num_img:]
            x1_proto=x1[-self.num_img:]
            

            
            online_outputs_proto=[el[-self.num_img:] for el in online_outputs]
            global_outputs_proto=[el[-self.num_img:] for el in global_outputs]
            
            
            x=x[:-self.num_img]
            x1=x1[:-self.num_img]
            
            online_outputs=[el[:-self.num_img] for el in online_outputs]
            global_outputs=[el[:-self.num_img] for el in global_outputs]
            

            activation_loss=self.get_activation_loss(online_outputs,global_outputs,x,x1)
            activation_loss_proto=self.get_activation_loss(online_outputs_proto,global_outputs_proto,x_proto,x1_proto)
            
            return torch.cat((x,x_proto),dim=0),activation_loss,activation_loss_proto
